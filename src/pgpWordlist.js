...
/**
 * Encode bytes to PGP words using RFC 1751 format
 * First word for odd bytes, second word for even bytes
...
function encodeToPgp(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const wordPair = pgpWordlist[hex];
    
    if (!wordPair || wordPair.length !== 2) {
      throw new Error(`Invalid wordlist entry for byte ${hex}: missing word pair`);
    }
    
    // First word for odd bytes, second for even bytes
    const word = (byte % 2 === 1) ? wordPair[0] : wordPair[1];
    words.push(word);
  }
  return words.join(' ');
}
...
/**
 * Decode PGP words back to bytes using RFC 1751 format
 * Validates that words are in correct odd/even positions
 * @param {string} encoded - Space or dash separated PGP words
 * @returns {Buffer} Decoded data
 * @throws {Error} If a word is unknown or in wrong position
 */
function decodeFromPgp(encoded) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    for (const [hexByte, wordPair] of Object.entries(pgpWordlist)) {
      if (!wordPair || wordPair.length !== 2) {
        throw new Error(`Invalid wordlist entry for ${hexByte}: missing word pair`);
      }
    ...
    if ((foundByte % 2) !== expectedParity) {
      throw new Error(`Invalid word position: "${word}" is for ${expectedParity === 1 ? 'odd' : 'even'} bytes at position ${i}`);
    }
